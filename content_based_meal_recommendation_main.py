...
def cold_start(sample_data, df_food_data, user_no):
    ...
    df_predict = update_preferences(df_food_data, df_user_ratings, int(user_no))

    return predict(df_predict, df_food_data, user_no)


def update_preferences(df_food_data, user_pref_df, user_id):
    ...
    # TODO: we need to find the normalized term frequency of each recipe-ingredient (square root each term in the matrix)
    df_ingredient_normalized = df_ingredient

    # Now we will update the user preferences and save it
    # If the preferences don't exist, we will create a new dataframe
    df_user_ratings = pd.DataFrame(columns=['user_id', 'food_id', 'rating'])
    ...
    updated_df.to_csv(os.path.join(os.getenv("DATA_SCIENCE_DIR"), 
                                    "content_based_meal_recommendations", "data", 'user_preferences.csv'), index=False)
    ...
    # Open the model and update the values for our user
...
if __name__ == "__main__":
    ...
    # Offline train if a model doesn't exist
    if (not os.path.exists(os.path.join(os.getenv("DATA_SCIENCE_DIR"), 
                                        "content_based_meal_recommendations", "data", "tfidf_model.pkl"))):
        train()
